<template>
    <div class="row m-0 p-4">
        ...
        <div class="col-md-6">
            ...
            <button @click="editBag()" class="btn btn-primary form-control rounded-1 mt-4"><span class="fa fa-shopping-bag"></span> Update Bag</button>      
        </div>
    </div>
</template>
<script>
export default {
    props:['id', 'index', 'toEdit'],
    data(){
        return{
            product:{},
            pictures:{main:""},
    ...
    methods:{
        ...
        async editBag(){
            if(this.currentSize == ''){
                ...
                await axios.post('/editCart', {product:this.product,color:this.currentColor,size:this.currentSize,quantity:1,index:this.index})
                .then( response =>{
                    this.getCart()
                    this.$notify({
                        group: 'foo',
        ...
        async getProduct(){
            ...
            .then( response =>{
                ...
                this.currentColor = this.toEdit.color
                this.currentSize = this.toEdit.size
